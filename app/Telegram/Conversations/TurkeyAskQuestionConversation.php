...
class TurkeyAskQuestionConversation extends Conversation
{
    /**
     * @param Nutgram $bot
    ...
    {
        $bot->sendMessage(__('telegram.turkey.question.start'));
    ...
    public function askQuestion(Nutgram $bot): void
    {
        ...
        if ($questionText) {
            ...
            $bot->sendMessage(__('telegram.turkey.question.end', [
                'command' => self::getName(),
                'link' => addSubdomainToUrl(route('faqs.index'), "turkey"),
            ]));
            $this->end();
            return;
        }

        $bot->sendMessage(__('telegram.turkey.question.unsupported'));
        $this->start($bot);
    }
    ...
    {
        return __('telegram.turkey.question.description');
    }
}
